...
class _SplitRow<L: RowType, R: RowType>: Row<SplitRowCell<L,R>> where L: BaseRow, R: BaseRow{
    ...
    private(set) var valueChanged = Set<SplitRowTag>()

    override var value: SplitRowValue<L.Cell.Value, R.Cell.Value>?{
    ...
    func subscribe<T: RowType>(onChange row: T) where T: BaseRow{
    ...
    func subscribe<T: RowType>(onCellHighlightChanged row: T) where T: BaseRow{
...
final class SplitRow<L: RowType, R: RowType>: _SplitRow<L,R>, RowType where L: BaseRow, R: BaseRow{}
